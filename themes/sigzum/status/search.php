...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col mb-2 mt-2">
            <section class="card border-default">
                <header class="card-header bg-default">
                    <h1 class="card-title h6 mb-1 mt-1">Total de Registros Encontrados: <?= ($statusTotal ?? "") ?></h1>
                </header>
                        ...
                        <tbody>
                            ...
                            ?>
                                    <tr>
                                        ...
                                        <td class="align-middle"><span class="p-1 rounded <?= ($statusItem->class_color ?? "") ?>"><?= ($statusItem->name ?? "") ?></span></td>
                                        <td class="align-middle"><?= ($statusItem->company()->full_name ?? "") ?></td>
                                        <td class="align-middle"><?= date_fmt($statusItem->created_at) ?></td>
                                        <td class="align-middle table-acao text-center">
                                            <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#modal_view_<?= (md5($statusItem->id) ?? "") ?>" title="Visualizar"><i class="fa fa-eye"></i></button>
                                            <a class="btn btn-primary btn-sm" href="<?= url("status/update/{$statusItem->id}") ?>" title="Editar"><i class="fa fa-pencil-alt"></i></a>
                                            <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal_remove_<?= (md5($statusItem->id) ?? "") ?>" title="Excluir"><i class="fa fa-trash"></i></button>
                                        </td>
                                    </tr>
                            <?php
                                    $qtd++;
    ...
    <!-- novo-registro -->
    <?php $this->insert("status/register", ["companies" => $companies]) ?>
    ...
    if (isset($status) && is_array($status)) :
        foreach ($status as $statusItem) :
            $this->insert("status/view", ["status" => $statusItem]);
        endforeach;
    endif;
    ?>
    ...
    if (isset($status) && is_array($status)) :
        foreach ($status as $statusItem) :
            $this->insert("status/remove", ["status" => $statusItem]);
        endforeach;
    endif;
    ?>